...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            // Branch change event - Load shifts for selected branch
            $('#branch_code').on('change', function() {
                ...
                $.ajax({
                    url: "{{ url('http://attendance2.localhost.com/api/group_manage/shifts-by-branch') }}/" +
                        branchCode,

                    method: "GET",
                    // crossDomain: true, // Add this
                    beforeSend: function() {
                        shiftDropdown.prop('disabled', true).html(
                    ...
                    success: function(response) {
                        if (response.shifts && response.shifts.length > 0) {
                            ...
                            response.shifts.forEach(shift => {
                                options +=
                                    `<option value="${shift.id}">${shift.shift_name_en}</option>`;
                            });
                    ...
                    error: function() {
                        shiftDropdown.html('<option value="">Error loading shifts</option>')
                            .prop('disabled', false);
                    }
                });
            });
            ...
            $('#btnSave').on('click', function(e) {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        branch_code: branch_code, // For validation
                        shift_id: shift_id,
